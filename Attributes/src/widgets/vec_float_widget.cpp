...
VecWidget::VecWidget(VecFloatAttribute *p_attr, QWidget *parent)
...
int VecWidget::get_hovered_point_index(const QPointF &pos)
{
...
float VecWidget::map_ypos_to_value(float ypos)
{
...
float VecWidget::map_value_to_ypos(float value)
{
...
void VecWidget::mouseMoveEvent(QMouseEvent *event)
{
...
void VecWidget::mousePressEvent(QMouseEvent *event)
{
...
void VecWidget::mouseReleaseEvent(QMouseEvent *event)
{
  if (event->button() == Qt::LeftButton)
  {
    if (this->moving_point_index != -1)
...
void VecWidget::paintEvent(QPaintEvent *event)
{
  Q_UNUSED(event);
...
void VecWidget::update_attribute_from_widget()
{
...
void VecWidget::update_widget_from_attribute()
{
  this->qpoints.clear();
...
VecFloatWidget::VecFloatWidget(VecFloatAttribute *p_attr) : p_attr(p_attr)
{
  ...
  // buttons
  {
    ...
    this->connect(button, &QPushButton::pressed, this->vec_widget, &VecWidget::new_value);
  }
